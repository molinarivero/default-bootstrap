								...
								{foreach from=$groups key=id_attribute_group item=group}
									{if $group.attributes|@count}
										<fieldset class="attribute_fieldset">
											<label  {if $group.group_type != 'color' && $group.group_type != 'radio'}for="group_{$id_attribute_group|intval}"{/if}><span style="color:red">Encuentre aqui los diferentes formatos{*{$group.name|escape:'html':'UTF-8'}*}&nbsp;</span></label>
					...
					{if $smarty.foreach.attachements.iteration %3 == 1}<div class="row">{/if}
						<div class="col-lg-4">
							...
							<a  href="{$link->getPageLink('attachment', true, NULL, "id_attachment={$attachment.id_attachment}")|escape:'html':'UTF-8'}">
								<img src="/img/fichaTecnica.png" alt="" width="112" height="29">
							</a>
                            ({Tools::formatBytes($attachment.file_size, 2)})
							<hr />
						</div>
