		...
		<fieldset>
			...
			<div class="clearfix">
				<div class="col-xs-12 col-md-3">
                  ...
                  <p class="form-group">
 <label for="email">{l s='Rut'}</label>
 <input class="form-control grey" type="text" id="rut" name="rut" value="{if isset($rut)}{$rut|escape:'htmlall':'UTF-8'|stripslashes}{/if}" />      
</p>
                  <p class="form-group">
 <label for="email">{l s='Nombre Cliente'}</label>
 <input class="form-control grey" type="text" id="name" name="name" value="{if isset($name)}{$name|escape:'htmlall':'UTF-8'|stripslashes}{/if}" />      
</p>
<p class="form-group">
 <label for="email">{l s='Télefono Contacto'}</label>
						...
						{if (!isset($customerThread.id_order) || $customerThread.id_order > 0)}
							<div class="form-group selector1">
								...
								{elseif $customerThread.id_order|intval > 0}
									<input class="form-control grey" type="text" name="id_order" id="id_order" value="{if isset($customerThread.reference) && $customerThread.reference}{$customerThread.reference|escape:'html':'UTF-8'}{else}{$customerThread.id_order|intval}{/if}" readonly />
								{/if}
							</div>
						{/if}
						{if isset($is_logged) && $is_logged}
							<div class="form-group selector1">
								...
								{elseif $customerThread.id_product > 0}
									<input  type="hidden" name="id_product" id="id_product" value="{$customerThread.id_product|intval}" readonly />
								{/if}
							</div>
						{/if}
					{/if}
